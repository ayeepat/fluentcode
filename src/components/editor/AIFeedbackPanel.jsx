...
import { CheckCircle, XCircle, Lightbulb, Sparkles, ArrowUp } from "lucide-react";

const QUICK_PROMPTS = ["Give me a hint", "Why is this wrong?", "Explain the concept"];
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;

export default function AIFeedbackPanel({ lesson, userCode, feedback, language }) {
...
Be concise, warm, and clear. Never reveal the full solution — guide instead. 2–4 sentences max unless the concept truly requires more.`;
      ...
      <div className="px-4 pb-3 flex flex-col gap-1.5 shrink-0">
        {QUICK_PROMPTS.map((p) => (
          <button
            ...
            className="text-left text-xs text-zinc-500 hover:text-zinc-900 px-3 py-2 rounded-xl border border-zinc-100 hover:border-zinc-300 transition-all duration-150"
          >
            {p}
          </button>
        ))}
  ...
  return (
    <div className="flex flex-col h-full bg-white border-l border-zinc-100">
      ...
      {/* Quick prompts — only when no messages and has feedback */}
